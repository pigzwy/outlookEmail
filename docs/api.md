...
| POST | `/api/external/accounts/tags` | API Key | JSON | 批量添加或移除普通邮箱账号标签 |
...
使用 API Key 为普通邮箱账号批量添加或移除标签。

#### 请求体

| 字段 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| `account_ids` | int[] | 是 | 要处理的账号 ID 列表 |
| `tag_id` | int | 是 | 标签 ID |
...
curl -X POST \
  ...
  -d '{"account_ids":[1,2],"tag_id":3,"action":"add"}' \
